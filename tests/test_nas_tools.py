"""Tests for NAS cataloguer tools."""
import tempfile
import os
...
def setup_module():
    get_tools(dry_run=True)  # ensure dry_run is on for all tests
...
def test_get_file_info(tmp_path):
    ...
    info = get_file_info.invoke({"path": str(f)})
...
def test_compute_hash_deterministic(tmp_path):
    ...
    r1 = compute_file_hash.invoke({"path": str(f)})
    r2 = compute_file_hash.invoke({"path": str(f)})
...
def test_find_duplicates(tmp_path):
    ...
    dupes = find_duplicates.invoke({"directory": str(tmp_path)})
    assert len(dupes) == 1
...
def test_delete_file_dry_run(tmp_path):
    ...
    assert result["status"] == "dry_run"
    assert f.exists()  # file should NOT have been deleted
